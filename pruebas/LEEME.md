...
| `probar_marca_equipo.js` | Cómo se ve una línea del detalle cuando el código **no pertenece al equipo** digitado (el caso real de `35062035` con `NUT-10105`). La aplicación siempre lo detectaba y frenaba el guardado, pero la marca iba en ámbar y el color de demarcación de la bandeja tapaba el borde: ahora la marca es **roja** —la misma de «no existe en el catálogo»— y la celda marcada **pierde el fondo de demarcación** para que el borde se note. Prueba los **dos caminos**, que son código distinto: el render inicial de la tabla y el repintado al teclear, midiendo con `getComputedStyle` sobre las funciones reales del `index.html`. Y que al corregir el equipo el color **vuelva** |
| `probar_usuario_inactivo.js` | Que un usuario **inactivo** no pueda hacer nada, y que la inactivación se pueda **deshacer**. Le pasa todos los `app.http()` del `index.js` real al envoltorio `candadoActivo` y exige 403 en todos menos en `GET me` —ese pasa a propósito: sin él la pantalla no podría explicar qué ocurrió—, y comprueba lo contrario, que un usuario ACTIVO no note nada. Contra un Postgres de verdad prueba los **tres candados** (correo protegido, uno mismo, último Administrador activo) y que reactivar devuelva a la persona **con su rol** y no con el de por defecto. Del lado de la pantalla: el aviso de acceso inactivo, que no se pida ningún catálogo, el checkbox «ver inactivos» —sin él la acción no se puede deshacer— y que una API que todavía no manda el campo `activo` **no bloquee a nadie** |
...
`probar_sima.js`, `probar_rol_impresion.js` y `probar_usuario_inactivo.js`
funcionan igual y comparten ese Postgres. La del rol Impresión además **corre las migraciones enteras** —todas
